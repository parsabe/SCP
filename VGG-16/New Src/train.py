...
class Trainer:
    ...
    def train(self, return_best_val_acc=False):
        ...
        log_file_path = f"logs_{self.optimizer_type}.txt"  # ✅ 1. log file path
        log_file = open(log_file_path, "w")                # ✅ 2. open log file

        def log(msg):                                      # ✅ 3. define logger
            print(msg)
            log_file.write(msg + "\n")
        ...
        for fold, (train_idx, val_idx) in enumerate(self.kf.split(range(len(self.train_loader.dataset)))):
            ...
            for epoch in range(self.num_epochs):
                ...
                for images, labels in train_loader:
                    ...
                    for i in range(self.num_classes):
                        ...
                        class_correct[i] += (predicted[idx] == i).sum().item()

                per_class_acc = class_correct / class_total
                per_class_acc_list.append(per_class_acc.tolist())
                ...
                if epoch_val_loss < best_val_loss:
                    ...
                    log(f"Val Loss Improved. Model saved at epoch {epoch+1}")  # ✅
                else:
                    epochs_no_improve += 1
            ...
            history["accuracy"] = history["train_acc"]
            history["loss"] = history["train_loss"]
            history["per_class_accuracy"] = per_class_acc_list
            history["per_class_loss"] = [[0.0 for _ in range(self.num_classes)] for _ in range(len(per_class_acc_list))]
        ...
        log(f"Estimated Test Error: {100 - avg_val_acc:.2f}%")
