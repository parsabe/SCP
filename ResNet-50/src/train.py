...
def mixup_data(x, y, alpha=0.2):
    ...
    batch_size = x.size()[0]
    # FIX: Ensure index is generated on the same device (GPU) as the data
...
class Trainer:
    ...
    def train(self, return_best_val_acc=False):
        ...
        for fold, (train_idx, val_idx) in enumerate(self.kf.split(range(len(self.train_loader.dataset)))):
            ...
            best_val_acc_for_fold = 0.0 # FIX: Track best accuracy
            epochs_no_improve = 0
            ...
            # PIN MEMORY helps CPU send data to GPU faster
            ...
            # 2. RESNET MODEL & PARALLELIZATION
            model = models.resnet18(weights=None)
            num_ftrs = model.fc.in_features
            model.fc = nn.Linear(num_ftrs, self.num_classes)
            
            if torch.cuda.device_count() > 1:
                print(f"Parallelizing ResNet across {torch.cuda.device_count()} GPUs!")
                model = nn.DataParallel(model)
                
            model = model.to(self.device)

            criterion = nn.CrossEntropyLoss(label_smoothing=0.01)
            # Pass the accurate length of the current fold's dataloader for OneCycleLR
            optimizer, scheduler = self.get_optimizer(model, steps_per_epoch=len(train_loader))
            ...
            for epoch in range(self.num_epochs):
                ...
                for images, labels in train_loader:
                    ...
                    # FIX: SGD's OneCycleLR must step after every BATCH
                    if self.optimizer_type == "SGD" and scheduler:
                        scheduler.step()
                ...
                print(f"Epoch [{epoch+1}/{self.num_epochs}], Train Loss: {epoch_train_loss:.4f}, Val Loss: {epoch_val_loss:.4f}, Val Accuracy: {val_accuracy:.2f}%")

                if epoch_val_loss < best_val_loss:
                    best_val_loss = epoch_val_loss
                    best_val_acc_for_fold = val_accuracy 
                    epochs_no_improve = 0
                ...
                # FIX: Adam's ExponentialLR must step after every EPOCH
                if self.optimizer_type == "Adam" and scheduler:
                    scheduler.step()
        ...
        print(f"\n=== Final Results ({self.optimizer_type}) ===")
